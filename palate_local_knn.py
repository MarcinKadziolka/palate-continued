...
def compute_local_palate_knn(
    ...
    if sigma is None:
        dists, _ = nn_train.kneighbors(train[:estimate_sigma_samples], n_neighbors=k)
...
def compute_global_palate_batched(train, test, gen, sigma, batch_size=500):
    ...
    for i in tqdm(range(0, N, batch_size), desc="Global PALATE batched"):
        ...
        d_tr = np.sum(
            (train[None, :, :] - batch[:, None, :]) ** 2, axis=2
        )  # [B, N_train]
        d_te = np.sum(
            (test[None, :, :] - batch[:, None, :]) ** 2, axis=2
        )  # [B, N_test]

        p_tr = np.exp(-d_tr / (2 * sigma**2)).mean(
            axis=1
        )  # density estimate per sample
